...
import { IngestedEventRecord } from '../types/index.js';

export const webhooksRouter = Router();

function parseWebhookRequest(req: Request): { rawBody: Buffer; payload: any } {
  let rawBody: Buffer;
...
webhooksRouter.post('/github/webhook', async (req: Request, res: Response) => {
  try {
    const { rawBody, payload } = parseWebhookRequest(req);
    const signature = req.headers['x-hub-signature-256'] as string;
    const deliveryId = (req.headers['x-github-delivery'] as string) || `gh_del_${Date.now()}`;
    ...
    if (!scopeCheck.inScope) {
      ...
      const skippedRecord: IngestedEventRecord = {
    ...
    const eventRecord: IngestedEventRecord = {
      id: `evt_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
      provider: 'github',
      eventType,
      deliveryId,
...
webhooksRouter.post('/slack/webhook', async (req: Request, res: Response) => {
  try {
    const { rawBody, payload } = parseWebhookRequest(req);
    ...
    let emailFetchMethod: 'api_lookup' | 'fallback_failed' = 'api_lookup';
...
webhooksRouter.post('/jira/webhook', async (req: Request, res: Response) => {
  try {
    const { rawBody, payload } = parseWebhookRequest(req);
    const rawSecretHeader = (req.headers['x-jira-webhook-secret'] as string) || '';
    ...
    if (!scopeCheck.inScope) {
      ...
      const skippedRecord: IngestedEventRecord = {
        ...
        deliveryId: (req.headers['x-atlassian-webhook-identifier'] as string) || `jira_del_${Date.now()}`,
    ...
    let userEmail: string | null = reporter.emailAddress || null;
    let userDisplayName = reporter.displayName || reporter.name || accountId;
    let username = reporter.name || accountId;
    let emailFetchMethod: 'payload_direct' | 'api_lookup' | 'fallback_failed' = userEmail
      ? 'payload_direct'
      : 'api_lookup';
