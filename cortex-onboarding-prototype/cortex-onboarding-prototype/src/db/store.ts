...
export interface DatabaseState {
  connectors: Record<SupportedProvider, ConnectorState>;
  persons: Record<string, CanonicalPerson>;
  events: IngestedEventRecord[];
  auditLogs: AuditLogEntry[];
}

function getDefaultConnectors(): Record<SupportedProvider, ConnectorState> {
...
class Store {
  ...
  // ─── Connector Methods ───────────────────────────────────────────────
  public getConnector(provider: SupportedProvider): ConnectorState {
  ...
  public getAllConnectors(): Record<SupportedProvider, ConnectorState> {
    return this.state.connectors;
  }

  public hasAnyConnected(): boolean {
    return Object.values(this.state.connectors).some(
      (c) => c.status === 'connected' || c.status === 'needs_reauth'
    );
  }

  public updateConnector(provider: SupportedProvider, patch: Partial<ConnectorState>): ConnectorState {
  ...
  public incrementEventCount(provider: SupportedProvider): void {
    const current = this.getConnector(provider);
  ...
  public getPerson(personId: string): CanonicalPerson | null {
    return this.state.persons[personId] || null;
  }

  public getAllPersons(): CanonicalPerson[] {
    return Object.values(this.state.persons);
  }

  public findPersonByEmail(email: string): CanonicalPerson | null {
  ...
  public findPersonByUsername(username: string): CanonicalPerson | null {
  ...
  public savePerson(person: CanonicalPerson): void {
  ...
  public addEvent(record: IngestedEventRecord): void {
  ...
  public getEvents(limit: number = 50): IngestedEventRecord[] {
