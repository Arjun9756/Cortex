...
export class IdentityService {
   ...
   */
  public resolveIdentity(input: ResolveIdentityInput): IdentityResolutionResult {
    ...
    for (const person of allPersons) {
      const existingIdentity = person.identities.find(
        (id) => id.provider === provider && id.externalId === externalId
      );
      ...
      if (existingIdentity) {
        ...
        if (cleanEmail && this.isMergeableEmail(cleanEmail)) {
          ...
          if (existingWithEmail && existingWithEmail.id !== person.id) {
            ...
            if (cleanUsername) existingIdentity.username = cleanUsername;

            for (const id of person.identities) {
              if (
                !existingWithEmail.identities.some(
                  (x) => x.provider === id.provider && x.externalId === id.externalId
                )
              ) {
    ...
    if (cleanEmail && this.isMergeableEmail(cleanEmail)) {
      ...
      if (existingPerson) {
        ...
        const newIdentity: ProviderIdentity = {
          provider,
          externalId,
          email: cleanEmail,
    ...
    const fallbackPerson: CanonicalPerson = {
      id: newPersonId,
      verifiedEmail: null,
      displayName: cleanDisplayName,
  ...
  private isMergeableEmail(email: string | null): boolean {
    ...
    if (genericPrefixes.includes(baseLocal) || genericPrefixes.includes(local)) return false;

    return true;
  }

  private isStrongUsername(username: string | null): boolean {
    ...
    if (generic.includes(clean)) return false;
    return true;
  }
}
