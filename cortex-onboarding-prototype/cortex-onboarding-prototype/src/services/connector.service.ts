...
export class ConnectorService {
   ...
   */
  public async getConnectorsStatus(): Promise<Record<SupportedProvider, ConnectorState>> {
    const connectors = store.getAllConnectors();
    const now = Date.now();

    for (const provider of ['github', 'slack', 'jira'] as SupportedProvider[]) {
      const conn = connectors[provider];
      if (conn.status === 'not_connected') continue;
   ...
   */
  private async autoRegisterWebhook(provider: SupportedProvider, baseUrl: string): Promise<WebhookConfig> {
    const webhookUrls: Record<SupportedProvider, string> = {
      github: `${baseUrl}/api/github/webhook`,
      slack: `${baseUrl}/api/slack/webhook`,
      jira: `${baseUrl}/api/jira/webhook`,
    };

    const signatureHeaders: Record<SupportedProvider, string> = {
   ...
   */
  public updateScopeRules(provider: SupportedProvider, rules: ScopeRules): ConnectorState {
    const updated = store.updateConnector(provider, {
      scopeRules: rules,
    });
    ...
    store.addAuditLog({
      ...
      message: `Updated monitoring scope for ${provider.toUpperCase()}: ${
        rules.allMonitored ? 'All items' : rules.monitoredItems.join(', ')
      }`,
      metadata: rules,
    });
   ...
   */
  public checkEventScope(
    provider: SupportedProvider,
    eventPayload: any
  ): { inScope: boolean; reason: string; matchedItem?: string } {
    ...
    if (provider === 'jira') {
      ...
      const projectKey =
        eventPayload?.issue?.fields?.project?.key ||
   ...
   */
  public async handleTokenFailure(
    provider: SupportedProvider,
    error: any
  ): Promise<{ refreshed: boolean; newAccessToken?: string }> {
    const conn = store.getConnector(provider);
   ...
   */
  public invalidateToken(provider: SupportedProvider): ConnectorState {
    const updated = store.updateConnector(provider, {
