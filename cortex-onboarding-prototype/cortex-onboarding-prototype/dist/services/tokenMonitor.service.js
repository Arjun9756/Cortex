...
export class TokenMonitorService {
    intervalTimer = null;
    CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // 24 hours (daily background check)
    start() {
        if (this.intervalTimer)
            return;
        console.log('[TokenMonitor] Starting daily proactive connector token health monitor');
        this.intervalTimer = setInterval(() => {
            this.checkAllTokensProactively().catch((err) => console.error('[TokenMonitor] Error during scheduled token scan:', err));
     ...
     */
    async checkAllTokensProactively() {
        ...
        for (const provider of providers) {
            ...
            let validation = isExpiredByTimestamp
                ? { valid: false, status: 'expired', errorMessage: 'Token timestamp has expired' }
