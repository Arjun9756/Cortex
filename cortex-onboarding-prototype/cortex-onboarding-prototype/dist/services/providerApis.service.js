...
export class ProviderApisService {
     ...
     */
    async fetchSlackUser(userId, token) {
        if (!token) {
            const err = new Error('No Slack access token available');
            err.code = 'NO_TOKEN';
            throw err;
        }
        // Check for simulated/expired test token
        if (token === 'EXPIRED_TOKEN' || token.startsWith('invalid_') || token.includes('_expired')) {
            const err = new Error('Slack API 401 Unauthorized: token_expired');
        ...
        try {
            ...
            if (!data.ok) {
                ...
                const isAuthError = ['token_expired', 'invalid_auth', 'not_authed', 'account_inactive', 'token_revoked'].includes(data.error);
                const err = new Error(`Slack API error: ${errorMsg}`);
     ...
     */
    async fetchJiraUser(accountId, token) {
        if (!token) {
            const err = new Error('No Jira access token available');
            err.code = 'NO_TOKEN';
            throw err;
        }
        // Check for simulated/expired test token
        if (token === 'EXPIRED_TOKEN' || token.startsWith('invalid_') || token.includes('_expired')) {
            const err = new Error('Jira API 401 Unauthorized: token_expired');
            err.code = 'TOKEN_EXPIRED';
            err.statusCode = 401;
            throw err;
        }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                const err = new Error(`Jira user lookup failed with HTTP ${res.status}`);
     ...
     */
    async testTokenValidity(provider, token) {
        if (!token) {
     ...
     */
    async attemptSilentRefresh(provider, refreshToken) {
        if (!refreshToken) {
