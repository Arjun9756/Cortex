                ...
                {details.suggestedBackups.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {details.suggestedBackups.map((candidate, idx) => {
                      const isCrossTraining = candidate.category === 'cross_training_candidate' || (candidate.sharedRepositories?.length === 0 && !candidate.category);
                      const isOverloaded = Boolean(candidate.isOverloaded || candidate.warningLabel);
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-start space-x-2.5">
                              <div className={`h-7 w-7 rounded-full border flex items-center justify-center font-bold text-xs shrink-0 mt-0.5 ${
                                isOverloaded
                          ...
                          <div className="flex items-center justify-between text-[10px] text-slate-400 border-t border-slate-800/80 pt-2">
                            <span>
                              Shared Tech: <strong className="text-purple-300">{candidate.sharedTechnologies.join(', ') || 'General stack'}</strong>
                            </span>
                            <span>
                              Capacity: <strong className={isOverloaded ? 'text-rose-300' : 'text-cyan-300'}>{candidate.capacityScore}%</strong>
                            </span>
                          </div>
                        </div>
                      );
